...
static void myuart_put_char(unsigned char ch);
...
static void __iomem *reg_uart_utxd;
static void __iomem *reg_uart_uts;
...
static int __init mychardev_init(void)
{
    ...
    // map uart
    reg_uart_utxd = ioremap(0x30860040, 4);
    reg_uart_uts  = ioremap(0x308600B4, 4);
...
static void myuart_put_char(unsigned char ch) {
    while (readl_relaxed(reg_uart_uts) != 0x60)
        cpu_relax();
    writel(ch, reg_uart_utxd);
}
